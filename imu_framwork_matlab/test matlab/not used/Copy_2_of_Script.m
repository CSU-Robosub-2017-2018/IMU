...
while(time < len)
    ...
    acc_2 = [acc_2; acc_1(time, :)];
    gyr_2 = [gyr_2; gyr_1(time, :)];
    
    lengthOfData = size(acc_2)-1;
    
    %filter acc
    if(lengthOfData(1) >= bufferSize)
        
        acc  = acc_2(1 : bufferSize, :);
        gyr = gyr_2(1 : bufferSize, :);
...
plot(linAcc)
        ...
        filtered_pos_bank = [filtered_pos_bank; linPosHP];
