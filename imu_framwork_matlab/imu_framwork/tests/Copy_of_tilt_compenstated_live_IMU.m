...
for i = 1:length(gyr)
    ...
    if(abs(tcAcc(1,1))<=limit)
        tcAcc(1,1) = 0;
    end

    if(abs(tcAcc(1,2))<=limit)
        tcAcc(1,2) = 0;
    end

    if(abs(tcAcc(1,3))<=limit)
        tcAcc(1,3) = 0;
    end
...
%             filtered_vel = filtfilt(b, a, filtered_vel);
            ...
            filtered_vel = mean(filtered_vel);
            filtered_vel_bank = [filtered_vel_bank; filtered_vel];

            raw_pos = raw_pos(1,:) + filtered_vel*samplePeriod;
...
%                 filtered_pos = filtfilt(b, a, filtered_pos);
                filtered_pos = mean(filtered_pos);
                
                filtered_pos_bank = [filtered_pos_bank; filtered_pos];
            end
        end
    end
end
